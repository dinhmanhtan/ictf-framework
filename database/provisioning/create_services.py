...
def register_service(db_api_url_base, db_secret, service_name, service_info):
    ...
    result = requests.post(
        db_api_url_base + "/upload/new", data=data, params={"secret": db_secret}
    )
    ...
    if response["result"] == "success":
        ...
        data = {
            ...
            "container_name": service_info.get("container_name", None),
            "can_restart": service_info.get("can_restart", 0),
        }
        result = requests.post(
            db_api_url_base + "/service/new", data=data, params={"secret": db_secret}
        )
        response = result.json()
            ...
            for script in os.listdir(scripts_path):
                if os.path.isfile(os.path.join(scripts_path, script)):
                    ...
                    if script_type in {"setflag", "getflag", "benign", "exploit"}:
                        print(f"Uploading script  [{script_type}]")
                        data = {
                            "upload_id": upload_id,
                            "filename": script,
                            "type": script_type,
...
def create_service(
    ...
    if not os.path.isfile(service_yaml):
        raise Exception(
            "Could not find service file for %s, tried %s. Skipping."
    ...
    if service_name != service_info["service_name"]:
        print(
            "{} ({})!= {} - Path of service doesn't match the name listed in info.yaml... Please change info.yaml manually and verify tgz is updated appropriately. ".format(
                service_name, service_path, service_info["service_name"]
            )
        )
        exit("100")
    ...
    service_info["can_restart"] = can_restart
    service_info["container_name"] = container_name
    return register_service(
        db_api_url_base, db_secret, service_info["service_name"], service_info
    )
...
def create_all_services(
    db_api_url_base, db_secret, services_info, services_folder_path
):
    processed = dict()
    for service in services_info:
        ...
        if service_path not in processed:
            if service_state != "enabled":
                ...
                continue
        can_restart = service.get("can_restart", 0)
        container_name = service.get("container_name", None)
        print(service,can_restart,container_name)
        create_service(
            db_api_url_base,
            db_secret,
            service_path,
