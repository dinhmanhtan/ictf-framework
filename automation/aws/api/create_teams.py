...
def get_all_teams(db_api_base_url, db_secret):
    ...
    try:
        ...
        raise Exception(
            "Invalid JSON returned from /teams/info endpoint: {}".format(result.content)
        ) from ex
    # print(response)
...
def create_team(
    ...
    result = requests.post(
        db_api_base_url + "/team/add_direct", data=data, params=params
    )
    print(result.content)
    response = result.json()
    if response["result"] == "success":
        print(team_email, team_password)
    ...
    for team in game_config["teams"]:
        ...
        if str(team["id"]) in current_teams:
            ...
            if cur_team["name"] != team["name"]:
                print(
                    "Already a team in the DB with the requested id %s but with a different name %s != %s"
                    % (team["id"], team["name"], cur_team["name"])
                )
                continue
            if cur_team["email"] != team["email"]:
                print(
                    "Already a team in the DB with the requested id %s but with different email %s != %s"
                    % (team["id"], team["email"], cur_team["email"])
                )
                continue
