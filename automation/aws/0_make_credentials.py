...
"""
    ...
    def sign(base, mode, name):
        subprocess.run(["openssl", "genrsa", "-out", base + ".key", "4096"], check=True)
        ...
        if mode == "ca":
            subprocess.run(
                [
                    ...
                    "-req",
                    "-signkey",
                    VPNKEYS_FOLDER + "/ca.key",
                    "-extfile",
                    VPNKEYS_FOLDER + "/ca.ext",
                    "-extensions",
                    "my_vpn_ca",
                    "-in",
                    base + ".csr",
            ...
            subprocess.run(
                [
                    ...
                    "-CAcreateserial",
                    "-CA",
                    VPNKEYS_FOLDER + "/ca.crt",
                    "-CAkey",
                    VPNKEYS_FOLDER + "/ca.key",
                    "-CAserial",
                    VPNKEYS_FOLDER + "/ca.srl",
                    "-extfile",
                    VPNKEYS_FOLDER + "/ca.ext",
                    "-extensions",
                    "my_vpn_" + mode,
    ...
    if not os.path.exists(VPNKEYS_FOLDER + "dh.pem"):
        subprocess.run(
            ["openssl", "dhparam", "-out", VPNKEYS_FOLDER + "dh.pem", "2048"],
            check=True,
        )
...
def create_credentials(num_teams, num_routers, router_public_ip):
    ...
    make_vpn_keys(4, num_routers, router_public_ip)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "game_config_file", type=str, help="Path to the game config file"
    )
    ...
    create_credentials(
        len(game_config["teams"]),
        game_config["game_info"]["num_routers"],
        game_config["game_info"]["router_public_ip"],
    )
